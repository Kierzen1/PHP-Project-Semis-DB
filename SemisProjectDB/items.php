...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'insert') {
        ...
        $stmt = $conn->prepare("INSERT INTO items (isbn, title, author, genre, price, booktype) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $isbn, $title, $author, $genre, $price, $booktype);
    ...
    if ($_POST['action'] == 'edit' && isset($_POST['id'])) {
        ...
        $stmt = $conn->prepare("UPDATE items SET isbn = ?, title = ?, author = ?, genre = ?, price = ?, booktype = ? WHERE i_id = ?");
        $stmt->bind_param("sssssii", $isbn, $title, $author, $genre, $price, $booktype, $i_id);
...
<body style="margin: 0;">
    ...
    <form method="post" action="">
        ...
        <input type="text" name="isbn" placeholder="ISBN:"style="width: 100%; padding: 4px;">
        <input type="text" name="title" placeholder="Title:"style="width: 100%; padding: 5px;">
        <input type="text" name="author" placeholder="Author:" style="width: 100%; padding: 5px;">
		<input type="text" name="genre" placeholder="Genre:"style="width: 100%; padding: 5px;">
        <input type="text" name="price" placeholder="Price:" style="width: 100%; padding: 5px;">
        <input type="text" name="booktype" placeholder="Booktype" style="width: 100%; padding: 5px;">
