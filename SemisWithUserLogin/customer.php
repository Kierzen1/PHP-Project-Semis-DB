...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"])) {
    if ($_POST["action"] == "insert") {
        ...
        $stmt = $conn->prepare(
            "INSERT INTO customer  (c_name, c_email, c_address, c_status) VALUES (?, ?, ?, 'active')"
        );
    ...
    if (
        ...
        $stmt = $conn->prepare(
            "UPDATE customer SET c_name = ?, c_email = ?, c_address = ?, c_status = ? WHERE c_id = ?"
        );
        $stmt->bind_param(
            "ssssi",
            $c_name,
        ...
        if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["search"])) {
            $search = $_GET["search"];
            $sql = "SELECT * FROM customer WHERE c_name LIKE '%$search%'";
        } else {
...
<table>
 ...
 if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["search"])) {
     $search = $_GET["search"];
     $sql = "SELECT * FROM customer WHERE c_name = '$search'";
 } else {
     $sql = "SELECT * FROM customer";
 }
