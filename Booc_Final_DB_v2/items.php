...
$conn = new mysqli("localhost", "root", "", "dbms_booc");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"])) {
    if ($_POST["action"] == "insert") {
        ...
        $stmt = $conn->prepare("INSERT INTO items (isbn, title, author, genre, booktype, stocks, price) VALUES (?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param("sssssis", $isbn, $title, $author, $genre, $booktype, $stocks, $price);
    ...
    if (
        ...
        $stmt = $conn->prepare("UPDATE items SET isbn = ?, title = ?, author = ?, genre = ?, booktype = ?, stocks = ?, price = ? WHERE i_id = ?");
		$stmt->bind_param(
            "sssssiii",
            $isbn,
            $title,
...
<table>
 ...
 ?>
    <?php if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "Booktype: <input type='text' name='booktype' value='" .
                $row["booktype"] .
                "'><br>";
			echo "Stocks: <input type='text' name='stocks' value='" .
                $row["stocks"] .
                "'><br>"; 
    ...
    <h4>Add Items</h4>
		...
		<input type="text" name="stocks" id = "stocks"placeholder="Stocks:"><br>
    ...
    if (isbn === "" || title === "" || author === ""|| genre ===""|| booktype ===""|| stocks===""|| price==="") {
