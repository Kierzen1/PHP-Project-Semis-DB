...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"])) {
    if ($_POST["action"] == "insert") {
        // Insert customer code remains unchanged
    }
    ...
    if ($_POST["action"] == "delete") {
        ...
        $order_check_query = "SELECT COUNT(*) AS order_count FROM orders WHERE c_id = $c_id";
        $order_check_result = $conn->query($order_check_query);
        $order_count = $order_check_result->fetch_assoc()['order_count'];

        if ($order_count > 0) {
            echo "Error: Customer has associated orders and cannot be deleted.";
        } else {
